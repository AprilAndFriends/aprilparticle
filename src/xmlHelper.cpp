...
const char* xml_node::find_prop(const char* property, bool ignoreError)
{
	...
	if (!ignoreError)
	{
		throw April::XMLPropertyNotExistsException(property, this);
	}
	return 0;
}
...
xml_doc::xml_doc(chstr filename)
{
	...
	if (this->doc == NULL)
	{
		throw April::GenericException("Unable to parse xml file '" + filename + "', document does not exist or is invalid");
	}
}
...
xml_node* xml_doc::root(chstr rootElementQuery)
{
	...
	if (root == NULL)
	{
		...
		xmlFreeDoc(this->doc);
		throw April::GenericException("No root node found in xml file '" + docname + "'");
	}
	if (rootElementQuery != "" && *root != rootElementQuery.c_str())
	{
		hstr docname = (char*)this->doc->URL;
		xmlFreeDoc(this->doc);
		throw April::GenericException("xml root node type is not '" + rootElementQuery + "' in xml file '" + docname + "'");
	}
	return root;
}
